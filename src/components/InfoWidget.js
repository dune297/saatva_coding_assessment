...
export default class InfoWidget extends Component {
	...
	addToCartButton = () => {
		return <button className="addToCartButton" onClick={this.props.incrementCartCount}>Add To Cart</button>
	};
	...
	additionalInfo = (selected, mattressData) => {
		...
		return (
			<div className="additionalInfo"> 
				...
				<p className="selectedPrice"> {new Intl.NumberFormat('en-US', currencyFormat)
					.format(selectedMattress.price)} </p>
			</div>
	)};
	...
	render() {
		...
		const buttonConfig = {
			labels: Object.keys(mattressData).map(mattress => mattressData[mattress].name),
		...
		return (
			<div>
				...
				<ButtonGroup buttonConfig={buttonConfig} 
					handleSelect={handleSelect}></ButtonGroup>
