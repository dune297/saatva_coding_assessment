	...
	carouselImage = () => {
		...
		return (
			<div className="imageContainer" 
				style={{ backgroundImage: `url(../src/resources/${mattressData[selected].imageFileName})` }}>
			</div>
	)};

	handleSelect = (selectedLabel) => {
		const mattressData = this.state.mattressData;
		this.setState({ selected: Object.keys(mattressData)
			.filter(mattress => mattressData[mattress].name === selectedLabel) });
	};
			...
			<div className="app">
				...
				<div className="content">
					...
					<div className="infoWidget">
						<InfoWidget mattressData={this.state.mattressData} incrementCartCount={this.incrementCartCount}
							selected={this.state.selected} handleSelect={this.handleSelect}/>
					</div>
				</div>
			</div>	
		)	
